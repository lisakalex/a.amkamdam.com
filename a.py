...
def decompose_tags(read_file1):
    ...
    message = soup.find('section', class_='header__global-message')
    if message:
        message.decompose()
...
def main():
    ...
    for currentpath, folders, files in os.walk('html'):
        for file in files:
            if '.jpg' not in file and '.png' not in file and '.svg' not in file and '.gif' not in file:
                ...
                # # read_file = None
                # # for fl in files:
                # # for filepath in glob2.escape('html/about2') + '/**/*', recursive=True):
                # #     print(str(count_replace) + ' ' + filepath)
                # # for entry in os.scandir('html/about2'):
                # # ku = ''
                # # try:
                # #     ku = magic.from_file(filepath, mime=True)
                # # except Exception as e:
                # #     print(e)
                # #     pass
                # #
